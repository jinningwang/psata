"""
IEEE Xplore API Interface.

This module provides an Python interface to the IEEE Xplore API, facilitating the retrieval
and analysis of scientific publications.
It is developed based on the Python 3 Software Development Kit provided by IEEE Xplore and is
designed to support researchers in accessing and analyzing open-source tools used in scientific publications.

Requirements:
- Python version: 3.4 or higher
- An IEEE Xplore API Access Key
- The pycurl module for making HTTP requests to the IEEE Xplore API
- The certifi module for SSL certificate verification

Dependencies:
    - pycurl
    - certifi

Special notes on pycurl installation:
If run into ImportError: pycurl: libcurl link-time ssl backend (openssl) is different from compile-time ssl backend (none/other)
error, you may need to set the PYCURL_SSL_LIBRARY environment variable to the SSL backend you are using before installing pycurl.
For example, if you are using the OpenSSL SSL backend, you can set the environment variable as follows:
```bash
export PYCURL_SSL_LIBRARY=openssl  # Adjust based on your SSL backend
